...
API.job.test = (verbose) ->
  ...
  tests = [
    () ->
      ...
      result.second = API.job.limit 2000, 'API.job.running', undefined, 'JOBTEST'
      result.limitend = Date.now()
      result.limitdifference = result.limitend - result.limitstart
      return result.first? and result.second? and result.limitdifference > 1990 and result.first is (API.settings.job?.startup ? false) and result.first is result.second
  ]

  if API.settings.job?.startup is true
    tests.push () ->
      result.job = API.job.create {group: 'JOBTEST', processes:['API.job.running','API.job.running']}
