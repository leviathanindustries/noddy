...
job_meta = new API.es.coll({index:"job",type:"meta"});
job_job = new API.es.coll({index:"job",type:"job"});
job_process = new API.es.coll({index:"job",type:"process"});
job_result = new API.es.coll({index:"job",type:"result"});

API.addRoute('job', {
  get: {
    action: function() {
      if ( this.queryParams.apikey ) {
        var user = API.accounts.retrieve(this.queryParams.apikey);
        if (user) {
          var u = user._id;
          var j = job_job.insert({new:true,user:u});
          var b = [];
          var r = this.queryParams.refresh;
          Meteor.setTimeout(function() { API.job.create(b,u,r,j); }, 5);
  ...
  post: {
    ...
    action: function() {
      ...
      var quota = API.job.quota(this.userId);
  ...
  get: {
    ...
    action: function() {
      ...
      if ( !API.job.allowed(job,this.user) ) {
        ...
        var p = API.job.progress(this.urlParams.job);
        job.progress = p ? p : 0;
...
API.addRoute('job/:job/reload', {
  get: {
    //roleRequired: 'root',
    action: function() {
      return {status: 'success', data: API.job.reload(this.urlParams.job) }
    }
  }
});

API.addRoute('job/:job/progress', {
  get: {
    //roleRequired: 'root',
    action: function() {
      // return the info of the job - the job metadata and the progress so far
...
API.addRoute('job/:job/results', {
  get: {
    ...
    action: function() {
      ...
      var res = API.job.results(this.urlParams.job);
...
API.addRoute('job/jobs/reload', {
  get: {
    roleRequired: 'root',
    action: function() {
      return {status: 'success', data: API.job.reload() }
    }
  }
});
...
API.addRoute('job/processes/reset', {
  get: {
    //roleRequired: 'root',
    action: function() {
      return {status: 'success', data: API.job.reset() }
    }
  }
});

API.addRoute('job/process/:proc', {
  get: {
    //roleRequired: 'root',
    action: function() {
      return {status: 'success', data: API.job.process(this.urlParams.proc) }
    }
  }
});

API.addRoute('job/status', {
  get: {
    action: function() {
      return {
        status: 'success', 
...
API.addRoute('job/quota/:email', { // is quota info going to be held by the job system directly?
  get: {
    //roleRequired: 'root',
    action: function() {
      if ( API.accounts.auth('root',this.user) || this.user.emails[0].address === this.urlParams.email ) {
        return {status: 'success', data: API.job.quota(this.urlParams.email) }
      } else {
        return {statusCode:401, body:{}}
      }
    }
...
API.job.allowed = function(job,uacc) {
  return job.user === uacc._id || API.accounts.auth('job.admin',uacc);
}

API.job.quota = function(uid) {
  var acc = API.accounts.retrieve(uid);
  var email = acc.emails[0].address;
  var max = 100;
  var admin = API.accounts.auth('job.admin',acc);
  var premium = API.accounts.auth('job.premium',acc,false ); // will job api handle premium account concept?
  if ( admin ) {
    max = 500000;
  } else if ( premium ) {
    max = 5000;
  ...
  var j = job_job.find({$and:[{email:email},{
// a job runner
// create a job with an object:
/*
{
...
API.job.create = function(input,uid,jid) {
  var job = {user:uid};
  ...
  var user = API.accounts.retrieve(job.user);
...
if ( Meteor.settings.cron && Meteor.settings.cron.job_alertstuck ) {
  SyncedCron.add({
    ...
    job: API.job.alertstuck
  });  
}
if ( Meteor.settings.cron && Meteor.settings.cron.job_alertdone ) {
  SyncedCron.add({
    name: 'job_alertdone',
    schedule: function(parser) { return parser.recur().every(10).minute(); },
    job: API.job.alertdone
  });
}
if ( Meteor.settings.cron && Meteor.settings.cron.job ) {
  SyncedCron.add({
    name: 'job',
    schedule: function(parser) { return parser.recur().every(1).second(); },
    job: API.job.nextProcess
  });
}
