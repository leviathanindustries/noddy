...
leviathan_statement = new API.collection("leviathan_statement");
leviathan_score = new API.collection("leviathan_score");
leviathan_source = new API.collection("leviathan_source");

//API.es.make(leviathan_statement,'leviathan','statement'); // this does not work yet because es is not available at this point... refactor should fix
...
API.add('service/leviathan/statement/:sid/scores', {
  get: {
    action: function() {
      return API.service.leviathan.scores(this.urlParams.sid);
    }
  }
});
...
API.add('service/leviathan/statement/:sid/responses', {
  get: {
    action: function() {
      return API.service.leviathan.responses(this.urlParams.sid);
    }
  }
});
...
API.add('service/leviathan/statements', {
  ...
  post: {
    action: function() {
      var data;
      if ( JSON.stringify(this.bodyParams).length > 2 ) data = this.bodyParams;
      return API.es.query('POST','/leviathan/statement/_search',data);
    }
  }
});
...
API.add('service/leviathan/scores', {
  ...
  post: {
    action: function() {
      var data;
      if ( JSON.stringify(this.bodyParams).length > 2 ) data = this.bodyParams;
      return API.es.query('POST','/leviathan/score/_search',data);
    }
  }
});
...
API.add('service/leviathan/:uid', {
  get: {
    action: function() {
      return API.service.leviathan.user(this.urlParams.uid);
    }
  }
});
...
API.add('service/leviathan/:uid/statements', {
  get: {
    action: function() {
      return API.service.leviathan.statements(this.urlParams.uid);
    }
  }
});
...
API.service.leviathan.lid = function(prev) {
...
*/
API.service.leviathan.statement = function(sid,obj,uid,filters,imported) {
  if (obj === true) {
    ...
    if (!imported) {
      ...
      if (uid) {
        scr.uid = uid;
        var usr = API.accounts.retrieve(uid);
...
API.service.leviathan.responses = function(sid) {
  // get the chain of responses from this statement sid if any
}

API.service.leviathan.scores = function(sid,filters) {
  // get the scores for this statement sid, possibly filtered
}

API.service.leviathan.user = function(uid) {
...
API.service.leviathan.statements = function(uid) {
...
API.service.leviathan.import.url = function(opts) {
  var src = HTTP.call('GET', opts.url).content;
  ...
  var content = API.convert.xml2txt(undefined,src);
  var ggl = API.use.google.cloud.language(content,'entities');
  var lthings = [];
  var things = [];
